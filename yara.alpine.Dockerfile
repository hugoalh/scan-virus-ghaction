...
ENV SCANVIRUS_GHACTION_TOOL_CLAMAV=false
...
# Install packages.
RUN apk --no-cache add ca-certificates curl git git-lfs icu-libs krb5-libs less libgcc libintl libssl1.1 libstdc++ lttng-ust@edge ncurses-terminfo-base tzdata userspace-rcu yara@edgetesting zlib
