...
RUN echo "deb http://deb.debian.org/debian/ sid main contrib" >> /etc/apt/sources.list && \
	...
	apt-get --assume-yes install apt-utils ca-certificates curl gss-ntlmssp hwinfo less libc6 libgcc1 libgssapi-krb5-2 libicu67 libssl1.1 libstdc++6 locales openssh-client zlib1g && \
