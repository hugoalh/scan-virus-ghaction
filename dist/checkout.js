...
await executeChildProcess(["git", "--no-pager", "clone", "--depth", "1", "https://github.com/hugoalh/scan-virus-ghaction-assets.git", "assets"], { cwd: process.env.SVGHA_ROOT }).then((result) => {
    if (!result.success) {
        console.error(result.stderr);
        process.exit(result.code);
    }
});
await executeChildProcess(["git", "--no-pager", "config", "--global", "--add", "safe.directory", process.env.SVGHA_ASSETS_ROOT]).then((result) => {
    if (!result.success) {
        console.error(result.stderr);
        process.exit(result.code);
    }
...
async function getProgramVersion(...inputs) {
    const result = await executeChildProcess(...inputs);
    if (!result.success) {
        console.error(result.stderr);
        process.exit(result.code);
    }
